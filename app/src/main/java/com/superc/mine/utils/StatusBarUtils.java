...
/********************************************************************
  @version: 1.0.0
  @description: 修改标题栏字体颜色,主要是将标题栏字体颜色设为深色
  @author: 杨帆
  @time: 2018/8/1 16:11
  @变更历史:
********************************************************************/
...
public class StatusBarUtils {
    ...
    private void setMIUIStatusBarDarkMode() {
        if (isMiUi) {
            Class<? extends Window> clazz = mActivity.getWindow().getClass();
            try {
                int darkModeFlag;
                ...
                extraFlagField.invoke(mActivity.getWindow(), darkModeFlag, darkModeFlag);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private int getStatusBarLightMode() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isMiUi) {
                result = 1;
            } else if (setMeiZuDarkMode(mActivity.getWindow(), true)) {
                result = 2;
    ...
    protected void setStatusBarDarkMode() {
        ...
        if (type == 1) {
            ...
            setMeiZuDarkMode(mActivity.getWindow(), true);
        } else if (type == 3) {
            mActivity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
        }
    }
